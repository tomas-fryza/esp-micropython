...
* [Part 2: Push button interrupt](#part2)
* [Part 3: ESP32 timers](#part3)
...
## Part 2: Push button interrupt

A push button interrupt on the ESP32 using MicroPython refers to setting up a hardware button (connected to a GPIO pin) so that when the button is pressed or released, a specific function (an interrupt handler) is triggered automatically and immediately — without having to constantly check the button state in your main code.
...
3. Save the module [`hw_config.py`](../modules/hw_config.py) from the previous lab to the ESP32 memory: **File > Save as... > MicroPython device**. Now, you can accsess classes defined within this Python file.
...
4. Create a new script file `button.py`, save it to *This computer*, and test the following application. The button will trigger an interrupt when pressed, and a message will be printed to the console. Note that the callback can be triggered when the pin value either falls (button pressed) or rises (button released).
   ...
   * `Pin.IRQ_RISING`: This triggers the interrupt when the pin goes from LOW to HIGH (button release).
   ...
   Here, the `button_handler(pin)` is the callback function that will be called when the button press causes the interrupt. The `pin` object is passed to the callback function automatically and represents the pin (GPIO 27) that triggered the interrupt.

5. Use class `Led` from `hw_config`, define a GPIO pin 2 and toggle the on-board LED anytime the button is pressed.
...
1. In Thonny, create a new script file `timer.py` and enter the following MicroPython code, which serves as a template for using timers. The `machine.Timer` class in MicroPython provides a way to set up and run a timer that calls a function (callback) either at regular intervals (periodically) or once after a delay.
...
2. Modify the template above, use class `Led` from `hw_config`, define a GPIO pin 2 and blink the on-board LED with a period of 1 sec. Try different Timer modes and setting.
...
2. Enhance the previous code by adding two other tasks with different periods.

<a name="challenges"></a>

## Challenges

1. Use a breadboard and jumper wires to connect two additional LEDs with resistors to ESP32 GPIO pins 25 and 26, using an active-high configuration.

   ![schema_leds](images/schema_leds.png)

   Modify the code so that each task controls the blinking of one LED.
